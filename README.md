...
这是一个基于 Web 的产品管理与防伪查询系统，用于验证产品的真伪。系统支持用户通过序列号验证产品，支持管理员后台管理，包括产品添加、编辑、删除、批量导入导出、模板管理、管理员管理、系统设置等功能。同时提供工厂模式 API，供生产系统直接提交产品数据。

**系统名称**可通过后台「系统设置」→「系统名称」动态配置，配置后将自动同步到前端查询页面（`index.html`）、管理员登录页面（`login.html`）和管理后台（`admin.html`）的标题与品牌展示区域。

## ✨ 功能特性

- ✅ **用户查询**：用户可以通过输入序列号（SN）验证产品真伪，支持 GET 和 POST 两种查询方式。
- 🔧 **管理员后台**：管理员可登录后台进行产品管理、类型管理、模板管理、管理员管理、系统设置等操作。
- 🏷️ **动态系统名称**：系统名称支持后台动态配置，所有页面（首页、登录页、后台）自动同步更新，无需修改代码。
...
- 🏷️ **自定义系统名称**：在后台「系统设置」→「系统名称」中修改名称，保存后所有页面自动更新。
...
```
查询项目/
├── 📄 index.html                        # 🏠 主页，用户查询界面（系统名称由 site_config 动态加载）
├── 📄 login.html                        # 🔑 管理员登录页面（系统名称由 site_config 动态加载）
├── 📄 admin.html                        # ⚙️ 管理员后台管理页面（系统名称由 site_config 动态加载）
├── 📄 db.sql                            # 🗄️ 数据库初始化脚本
...
│   ├── 📄 site_config.php               # 🌐 站点公共配置（含系统名称、备案号、版权信息等，供前端页面动态加载）
...
│   └── 📄 login.js                      # 登录页面逻辑
│
├── 📁 python/                           # 🐍 Python 工厂导入工具
│   ├── 📄 factory_import.py             # 工厂模式批量导入脚本
│   ├── 📄 factory_import_last.ini       # 上次导入参数缓存
│   └── 📄 factory_import.log            # 导入运行日志
│
├── 📁 images/                           # 🖼️ 静态图片资源
├── 📁 uploads/                          # 📤 用户上传文件存储
